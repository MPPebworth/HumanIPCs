...
cellnumber = table(MP_IPC3_2@active.ident)
object1 <- MP_IPC3_2
object1[["old.ident"]] <- Idents(object = object1)
NumCells <-dim(data1)[2]

start_time <- Sys.time()
allMarkers<- foreach(m = 1:length(cellnumber),.combine='rbind', .packages=c('foreach','Seurat','doParallel')) %do%{
  ...
  foreach(j=1:100, .combine='rbind', .packages=c('foreach','Seurat','doParallel')) %dopar%{
    ...
    foreach(i=1:100, .combine = 'rbind', .packages=c('Seurat')) %do% {
      ...
      Cluster <- rep(names(cellnumber)[m],dim(test)[1])
      test <- cbind(test,Iteration,Permute,Cluster)
...
write.csv(allMarkers, file ="PermuteNCells_AllMarkers.csv")
