#This code is for randomly selecting n-cells and running the SeuratFunction FindMarkers. 
#The idea is to test the p-values that would be generated by chance if a random set of cells were detected and defined as a cluster.

#I used the package PackRat to initialize an environment that has Suerat version 3 and supporting documents
packrat::init("Seuratv3", restart = TRUE, infer.dependencies = FALSE)

library(Seurat)
library(doParallel)

#Here I load my Seurat object that has my original single cell data. 
#The object within this RData file is called MP_IPC3_2. 
...
#Set up Parallel processes to speed up the results. 
cl <- makeCluster(10)
